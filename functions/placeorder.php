...
{   
        if(isset($_POST['placeOrderBtn']))
        {
            ...
            $totalPrice = 0;
             ...
             $tracking_no = "east".rand(1111,9999).substr($phone,2);
            ...
            if($insert_query)
            {
                ...
                foreach($query_run as $citem)
                {
                   ...
                   $price = $citem['selling_price'];
                     ...
                     $current_qty = $productData['qyt'];

                     $new_qty = $current_qty -  $prod_qty;

                     $updateQyt_query = "UPDATE products SET qty='$new_qty' WHERE id='$prod_id' ";
                     $updateQyt_query_run = mysqli_query($con, $updateQyt_query);
                }
                $deleteCartQuery = "DELETE FROM carts WHERE user_id='$userId' "; 
                $deleteCartQuery_run = mysqli_query($con, $deleteCartQuery);
                if($payment_mode == "COD"){
                
                    $_SESSION['message'] = "Orders places successfully!";
